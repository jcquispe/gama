...
class LogController extends Controller {
	...
	public function bienadmin()
	{
		...
		for($i = 1; $i <= 12; $i++){
	        $mes = \DB::table('conectados')->whereRaw('extract(month from created_at) = ?', [$i])->count();
	        $totConec[$i-1] = $mes;
        }
	...
	public function store(LoginRequest $request)
	{
		if(Auth::attempt(['us' => $request['usuario'], 'password' => $request['pass']])){
			...
			$idcon = Conectado::create([
						'ip' => getHostByName(getHostName()),
						'estado' => 'CONECTADO',
						'user_id' => Auth::user()->id
					]);

			Session::set('idcon', $idcon->id);

			if(Auth::user()->tipo == 'admin'){
				$conectados = Conectado::where('estado','CONECTADO')->where('created_at','<',date('Y-m-d'))->get();
				//echo '<pre>';print_r($conectados);die;
				foreach($conectados as $con){
					$con['estado'] = 'DESCONECTADO';
					$con->save();
				}
			}
			...
			switch(Auth::user()->tipo){
				...
				case 'admin': return Redirect::to('bienadmin');break;
	...
	{
		//echo Session::get('idcon');die;
		$conectado = Conectado::find(Session::get('idcon'));
